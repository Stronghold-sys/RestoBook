...
async function run() {
  console.log("Membaca file SQL migrasi Rotating IP...");
  const sqlPath = path.join(__dirname, 'migrations', 'create_rotating_ip_security_tables.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');

  console.log("Mengirim skema pelacakan identitas tingkat lanjut ke Database Supabase...");
  ...
  if (error) {
     ...
     if (err2) {
        ...
        console.log("SUKSES BESAR MELALUI JALUR ALTERNATIF! Migrasi Rotating IP Selesai.");
     }
  } else {
     console.log("SUKSES MUTLAK! Tabel-tabel pelacakan identitas tingkat lanjut berhasil dibuat.");
  }
