...
async function run() {
  const sql = `
    -- 1. Add cashback_amount to reward_redemptions table
    ALTER TABLE reward_redemptions ADD COLUMN IF NOT EXISTS cashback_amount DECIMAL(12,2) DEFAULT NULL;

    -- 2. Update existing reward_redemptions cashback_amount using rewards table
    UPDATE reward_redemptions rr
    SET cashback_amount = r.cashback_amount
    FROM rewards r
    WHERE rr.reward_id = r.id AND r.category = 'cashback' AND rr.cashback_amount IS NULL;

    -- 3. Re-create the redeem_reward_transaction function to record cashback_amount
  ...
  console.log("Running redemptions cashback_amount column migration...");
  const { data, error } = await supabase.rpc('exec_sql', { sql_string: sql });
  if (error) {
    console.error("Migration failed:", error);
    return;
  }
  console.log("Migration and redeem function updated successfully!");
}
