...
async function run() {
  console.log("MEMULAI MIGRASI BATAS PENUKARAN REWARD...");

  const sql = `
    -- 1. Tambahkan kolom redeem_limit dan redeem_limit_period ke tabel rewards
    ALTER TABLE public.rewards ADD COLUMN IF NOT EXISTS redeem_limit INT DEFAULT NULL;
    ALTER TABLE public.rewards ADD COLUMN IF NOT EXISTS redeem_limit_period TEXT DEFAULT 'all';

    -- 2. Tambahkan check constraint untuk redeem_limit_period
    ALTER TABLE public.rewards DROP CONSTRAINT IF EXISTS chk_redeem_limit_period;
    ALTER TABLE public.rewards ADD CONSTRAINT chk_redeem_limit_period CHECK (redeem_limit_period IN ('hour', 'day', 'week', 'month', 'all'));
    ...
    BEGIN
      ...
      SELECT title, description, category, min_points, stock, is_active, discount_percent, cashback_amount, expiry_days, redeem_limit, redeem_limit_period
      INTO v_reward_title, v_reward_desc, v_reward_category, v_reward_min_points, v_reward_stock, v_reward_active, v_reward_discount, v_reward_cashback, v_reward_expiry_days, v_redeem_limit, v_redeem_limit_period
      FROM rewards
      ...
      IF v_redeem_limit IS NOT NULL AND v_redeem_limit > 0 THEN
        ...
        WHERE customer_id = p_customer_id
          ...
          AND (
            ...
            OR (v_redeem_limit_period = 'hour' AND created_at >= date_trunc('hour', now()))
            OR (v_redeem_limit_period = 'day' AND created_at >= date_trunc('day', now()))
            OR (v_redeem_limit_period = 'week' AND created_at >= date_trunc('week', now()))
            OR (v_redeem_limit_period = 'month' AND created_at >= date_trunc('month', now()))
          );

        IF v_current_redemptions >= v_redeem_limit THEN
          IF v_redeem_limit_period = 'hour' THEN v_period_lbl := 'jam';
          ...
          IF v_redeem_limit_period = 'all' THEN
            RETURN json_build_object('success', false, 'error', 'Kuota penukaran reward ini telah habis (Maksimal ' || v_redeem_limit || 'x).');
          ELSE
            RETURN json_build_object('success', false, 'error', 'Kuota penukaran penuh (' || v_redeem_limit || 'x per ' || v_period_lbl || '). Silakan tunggu ' || v_period_lbl || ' berikutnya.');
          END IF;
        END IF;
      END IF;
  ...
  console.log("Migrasi batas penukaran reward selesai dengan sukses!");
  process.exit(0);
}
