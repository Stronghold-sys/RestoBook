...
async function run() {
  console.log("MEMULAI MIGRASI KOLOM IS_QUOTA_FREED DAN UPDATE FUNGSI REDEEM...");

  const sql = `
    -- 1. Tambahkan kolom is_quota_freed ke public.reward_redemptions
    ALTER TABLE public.reward_redemptions ADD COLUMN IF NOT EXISTS is_quota_freed BOOLEAN DEFAULT FALSE;

    -- 2. Update dan re-create fungsi redeem_reward_transaction
  ...
  console.log("Migrasi is_quota_freed sukses!");
  process.exit(0);
}
