...
async function run() {
  const sql = `
    ...
    BEGIN
      ...
      IF v_reward_category = 'voucher' OR v_reward_category = 'food' THEN
        -- Generate voucher code
        IF v_reward_category = 'food' THEN
          v_voucher_code := 'FREEFOOD-' || upper(substring(md5(random()::text), 1, 6));
          -- Insert new voucher
          INSERT INTO vouchers (code, discount_percent, usage_limit, used_count, max_usage_per_user, is_active, expires_at)
          VALUES (v_voucher_code, 100, 99999, 0, 1, true, now() + interval '30 days')
  ...
  console.log("Updating redeem_reward_transaction Supabase function...");
