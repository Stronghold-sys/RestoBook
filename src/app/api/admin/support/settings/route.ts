  ...
  try {
    ...
    if (error || !settings) {
      ...
      const defaultSettings = {
        ...
        order_chat_expiry_hours: 0,
        order_chat_expiry_minutes: 30,
        order_chat_expiry_seconds: 0,
        sla_hours_low: 48,
        sla_hours_medium: 24,
        sla_hours_high: 12,
        sla_hours_urgent: 4
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const {
      ...
      chat_expiry_minutes,
      chat_expiry_seconds,
      order_chat_expiry_hours,
      order_chat_expiry_minutes,
      order_chat_expiry_seconds,
      sla_hours_low,
      sla_hours_medium,
      sla_hours_high,
      sla_hours_urgent
    ...
    const { data: updatedSettings, error } = await supabase
      ...
      .update({
        ...
        order_chat_expiry_hours: Number(order_chat_expiry_hours ?? 0),
        order_chat_expiry_minutes: Number(order_chat_expiry_minutes ?? 30),
        order_chat_expiry_seconds: Number(order_chat_expiry_seconds ?? 0),
