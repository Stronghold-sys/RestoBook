...
// MD5 implementation for Edge Runtime
...
export async function POST(req: Request) {
  try {
    ...
    if (!DUITKU_MERCHANT_CODE || !DUITKU_API_KEY) {
      return NextResponse.json({ methods: STATIC_FALLBACK, warning: 'Menggunakan data fallback karena kredensial belum diatur' });
    }

    const datetime = getFormattedDate();
    const paymentAmount = String(amount || 10000);
    
    // Sandbox Signature: MD5(merchantCode + amount + datetime + merchantKey)
    const signature = md5(`${DUITKU_MERCHANT_CODE}${paymentAmount}${datetime}${DUITKU_API_KEY}`);
    ...
    try {
      ...
      if (data.paymentFee && data.paymentFee.length > 0) {
        ...
        console.warn('Duitku API returned empty or error, using fallback:', data);
        return NextResponse.json({ methods: STATIC_FALLBACK, warning: 'Duitku API Offline/Error', details: data });
      }
    } catch (e) {
