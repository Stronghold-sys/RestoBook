...
// MD5 implementation for Edge Runtime (since crypto.subtle might not support MD5)
...
export async function POST(req: Request) {
  try {
    ...
    // Duitku Get Payment Method Signature: MD5(merchantCode + paymentAmount + merchantKey)
    // Note: Duitku standard list often uses MD5
    const signature = md5(`${DUITKU_MERCHANT_CODE}${paymentAmount}${DUITKU_API_KEY}`);
    ...
    if (data.paymentFee) {
      ...
      return NextResponse.json({ 
        ...
        details: data,
        debug: { datetime, paymentAmount, signatureType: 'MD5' } 
      }, { status: 400 });
    }
