...
import { useToasterStore, toast, resolveValue, Toast } from 'react-hot-toast';
...
function ToastMessage({ toast: t }: { toast: Toast }) {
  ...
  const styles = {
    ...
    error: {
      ...
      icon: <XCircle className="w-5 h-5 shrink-0 text-rose-500" />
    },
    warning: {
    ...
    realtime: {
      ...
      icon: <Bell className="w-5 h-5 shrink-0 text-violet-500 animate-bounce" />
    }
  };
  ...
  return (
    <motion.div
      ...
      className={`w-[calc(100vw-32px)] sm:w-96 max-w-md bg-white/80 dark:bg-slate-900/85 backdrop-blur-xl border ${styleConfig.border} ${styleConfig.text} ${styleConfig.shadow} p-4.5 rounded-2xl flex items-start gap-3.5 relative overflow-hidden transition-all duration-300 pointer-events-auto`}
    >
      {/* Icon Area */}
      ...
      <div className="flex-1 min-w-0 pr-6">
        <p className="text-xs font-black tracking-wider uppercase opacity-55">
        ...
        <p className="text-sm font-bold mt-1 text-text-light dark:text-text-dark leading-relaxed">
          {messageText}
        </p>
      </div>
      ...
      <button
        ...
        className="absolute top-3.5 right-3.5 p-1 text-muted hover:text-text-light dark:hover:text-text-dark rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      >
        <X className="w-4 h-4" />
      </button>
...
export default function ModernToaster() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[99999] flex flex-col items-center gap-3 p-4 pointer-events-none mt-safe-top">
