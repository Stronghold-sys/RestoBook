// Pure JavaScript MD5 implementation (runs on Node.js build time, Edge, and browser)
export async function md5(message: string): Promise<string> {
  return md5Sync(message);
}

function md5Sync(message: string): string {
  const k = [
  ...
  for (let i = 0; i < utf8Encode.length; i++) {
    words[i >> 2] |= utf8Encode[i] << ((i % 4) * 8);
  }
  
  const byteLength = utf8Encode.length;
  words[byteLength >> 2] |= 0x80 << ((byteLength % 4) * 8);
  
  const wordCount = ((byteLength + 8) >> 6) * 16 + 14;
  while (words.length < wordCount) words.push(0);
  
  // Pad with length in bits
